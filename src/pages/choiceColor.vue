<template>
	<div class="container">
		...
		<div class="car_color_content">
			<div class="title">颜色</div>
			<ul class="color_list">
				<li class="typeItem" data-id="0" v-bind:class="[activeItem == 0 ? 'active' : '']" @click="checked(0,'不限')"><span class="radioDefault"></span><span class="text">不限</span></li>
				<li class="typeItem" v-for="item in data" :key="item.id" :data-id="item.id" v-bind:class="[activeItem == item.id ? 'active' : '']" @click="checked(item.id,item.name)"><span class="radioDefault"></span><span class="text">{{item.name}}</span><span class="color" v-bind:style="{background: item.colour}"></span></li>
			</ul>
		</div>
	</div>
</template>
...
<script>
	...
	export default{
		...
		methods:{
			loadData: function(){
				var _this = this;
				this.$fetchPost('/getCarColour',{token: this.token}).then(function(res){
					console.log(res)
					_this.data = res.data;
				})
			},
...
<style scoped>
	...
	.car_color_content .color_list li .color {
	    width: 0.26rem;
	    height: 0.26rem;
	    border-radius: 100%;
