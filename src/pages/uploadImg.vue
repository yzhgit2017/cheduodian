<template>
	<div>
		...
		<div class="upload_img_content">
			...
			<div class="img_list">
			    <div class="iw fmiw" v-for="(item,index) in uploadImgData" :key="index" v-bind:class="[fengmian == item ? 'active' : '']"  @touchstart="tsevent()" @touchend="teevent(index)">
			        <div class="uploadImgW"><img :src="http + item" class="carimg" style="height: 100%; opacity: 1;"></div>
...
<script>
	...
	export default{
		...
		mounted(){
			this.uploadImgData = this.publishData.data.photos;
			this.fengmian = this.publishData.data.fengmian;
		},
		methods: {
			choiceImg: function(){
			        	...
			        	if(this.num < xz){
			                ...
			                this.$fetchPost('/upImages',formdata).then(function(res){
								...
								if(res.error == 0){
									...
									that.uploadImgData.push(res.url);
									if(that.fengmian == ''){
										console.log(that.uploadImgData)
										that.fengmian = that.uploadImgData[0];
									}
								}
							})
							that.num++;
			...
			teevent: function(index){
		        ...
		        if(endTime - this.startTime > 500 && this.imgIndex != 0){
		        	...
		        	if(event.target.className == "close"){
		        		...
		        		this.fullsiu = this.uploadImgData[index];
			        	this.$singleImgZoom({
			...
			confirmSet: function(){
				if(this.caozuo == 'setfm'){
	   				this.fengmian = this.uploadImgData[this.imgIndex];
				}
   				if(this.caozuo == 'del'){
			...
			save: function(){
				...
				this.$store.commit('publishCondition/uploadImg',{fengmian: this.fengmian, photos: this.uploadImgData});
				this.$router.goBack();
			}
		}
	}
