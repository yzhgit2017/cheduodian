	...
	<div class="container">
		...
		<div class="biansuxiang">
			...
			<div class="range_list">
				<div class="biansu_item" v-for="(item,index) in cotyData" @click="choice(item.id,item.gear_box,index)" v-bind:class="[activeItem == index ? 'active' : '']"><label class="label1">{{item.gear_box}}</label><img src="../assets/images/bss.png"></div>
			</div>	
		</div>		
	</div>
</template>
...
<script>
	...
	export default{
		...
		methods: {
			loadRange: function(){
				var _this = this;
				this.$fetchPost('/getGearbox',{token: this.token}).then(function(res){
					console.log(res)
					_this.cotyData = res.data;
				})
			},
			choice: function(id,val,index){
