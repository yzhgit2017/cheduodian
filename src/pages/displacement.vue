...
<script>
	...
	export default{
		...
		methods: {
			loadRange: function(){
				var _this = this;
				this.$fetchPost('/getDisplacement',{token: this.token}).then(function(res){
					console.log(res)
			...
			choice: function(min,max,val,index){
     			...
     			if(this.$route.query.from == 'filter'){
     				this.$store.commit('filterCondition/displacement',{min: min, max: max, val: val});
     			}else{
     				this.$store.commit('vehicleList/displacement',{min: min, max: max, val: val});
     			}
